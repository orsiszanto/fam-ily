...
class TodoBloc extends Cubit<TodoState> {
  ...
  void loadTodoLists({required String groupId}) {
    emit(TodoLoading());

    _todoService
        .getTodoLists(groupId: groupId)
        .then((todoLists) {
  ...
  void startCreateDraft() {
    emit(TodoEditing(title: '', items: [], isEditMode: false));
  }

  void startEditDraft({required String groupId, required String todoListId}) {
    emit(TodoLoading());
    _todoService
        .getTodoListById(groupId: groupId, todoListId: todoListId)
        .then((todoList) {
          _todoService
              .getTodoItems(groupId: groupId, todoListId: todoListId)
              .then((items) {
                emit(
                  TodoEditing(
  ...
  void addDraftItem() {
    ...
    if (current is TodoEditing) {
      final updatedItems = List<TodoItem>.from(current.items)
        ..add(TodoItem(id: null, title: '', isDone: false));
  ...
  void updateDraftItemTitle({required int i, required String newTitle}) {
    final current = state;
    if (current is TodoEditing) {
      final updatedItems = List<TodoItem>.from(current.items);
  ...
  void updateDraftItemStatus({required int i}) {
    final current = state;
    if (current is TodoEditing) {
      final updatedItems = List<TodoItem>.from(current.items);
      updatedItems[i] = TodoItem(
  ...
  void deleteDraftItem({required int i}) {
    final current = state;
    if (current is TodoEditing) {
      final updatedItems = List<TodoItem>.from(current.items);
  ...
  void saveDraft({required String groupId, required String currentUserId}) {
    final current = state;
  ...
  void deleteTodoList({required String groupId, required String todoListId}) {
    _todoService
        .deleteTodoList(groupId: groupId, todoListId: todoListId)
        .then((_) {
          emit(TodoDeleted());
        })
        .catchError((error) {
